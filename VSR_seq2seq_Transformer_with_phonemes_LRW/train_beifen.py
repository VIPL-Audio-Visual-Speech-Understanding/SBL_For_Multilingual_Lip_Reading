...
def train_net(args):
    ...
    valid_loader = torch.utils.data.DataLoader(valid_dataset, batch_size=args.batch_size, collate_fn=pad_collate, pin_memory=True, shuffle=True, num_workers=args.num_workers)
...
def train(train_loader, model, optimizer, epoch, logger, k):
    ...
    for i, (data) in enumerate(train_loader):
        ...
        #padded_input = Variable(padded_input.cuda())
        #padded_target = Variable(padded_target.cuda())
        #print(padded_input)
        ...
        #output, _ = pad_packed_sequence(padded_input, batch_first=True, total_length=total_length)
        ...
        if padded_target.size(1) <= word_length:
            with torch.no_grad():
                ...
                for arr in preds.cpu().numpy():
                    preds = [char_list[one] for one in arr if one not in (sos_id, eos_id)]
