...
with open('chinese_phonemes_gai.txt', 'r') as file:
  ...
  for line in lines:
    ...
    for one in phonemes:
      if one not in chinese_phonemes:
        chinese_phonemes.append(one) 
        
print(chinese_phonemes, len(chinese_phonemes))

#chinese_phonemes = ['a', 'ai', 'an', 'ang', 'au', 'b', 'ei', 'en', 'eng', 'ii', 'i', 'ie', 'in', 'ing', 'uo', 'uu', 'ts', 'e1', 'ch', 'iii', 'ong', 'eu', 'u', 'd', 'dong', 'er', 'f', 'g', 'h', 'j', 'yu', 'yue', 'n', 'k', 'l', 'm', 'o', 'p', 'q', 'zh1', 's', 'sh', 't', 'w', 'x', 'y', 'z', 'zh']

english_phonemes = ['s', 'p', 'ii', 'k', 'i', 'ng', 'l', 'e', 'v', 'e1', 'a1', 'm', 'z', 'zh', 'o', 'r', 'eu', 't', 'ai', 'h', 'th', 'y', 'n', 'ch', 'ae', 'au', 'er', 'd', 'f', 'ei', 'w', 'a', 'oi', 'b', 'uu', 'g', 'sh', 'dh', 'u', 'zh1']
print('lrw_phonemes: ', english_phonemes, len(english_phonemes))

total_phonemes = []
total_phonemes = english_phonemes
for one in chinese_phonemes:
  if one not in english_phonemes:
      total_phonemes.append(one)
      
print(total_phonemes, len(total_phonemes))
...
with open('chinese_phonemes_gai.txt', 'r') as file:
    ...
    for line in lines:
        items = line.rstrip('\n').strip('').split('  ')
        chinese_phones[items[0]] = items[1]

lrw1000_phonemes = []
for one in lrw1000_words:
    phonemes = chinese_phones[one]
    items = phonemes.split(' ')
    for one in items:
        if one not in lrw1000_phonemes:
            #if one == 'o':
             #   print(one)
            lrw1000_phonemes.append(one)
            
print('lrw1000_phonemes: ',lrw1000_phonemes, len(lrw1000_phonemes))
for one in chinese_phonemes:
    if one not in lrw1000_phonemes:
        print(one)

total1 = ['a', 'ae', 'ai', 'an', 'ang', 'au', 'b', 'ch', 'd', 'dh', 'e', 'ei', 'e1', 'en', 'eng', 'er', 'eu', 'f', 'g', 'h', 'i', 'ie', 'ii', 'iii', 'in', 'ing', 'j', 'k', 'l', 'm', 'n', 'ng', 'o', 'oi', 'p', 'q', 'r', 's', 'sh', 't', 'ts', 'u', 'ong', 'uo', 'uu', 'v', 'a2', 'w', 'x', 'y', 'yu', 'yue', 'z', 'zh', 'zh1', 'th']
print(len(total1))
