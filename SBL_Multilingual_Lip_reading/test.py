...
total_phonemes = ['sos', 'eos', 's', 'p', 'ii', 'k', 'i', 'ng', 'l', 'e', 'v', 'e1', 'a1', 'm', 'z', 'zh', 'o', 'r',
                  'eu', 't', 'ai', 'h', 'th', 'y', 'n', 'ch', 'ae', 'au', 'er', 'd', 'f', 'ei', 'w', 'a', 'oi', 'b',
                  'uu', 'g', 'sh', 'dh', 'u', 'zh1', 'an', 'ang', 'en', 'eng', 'ie', 'in', 'ing', 'uo', 'ts', 'iii',
                  'ong', 'j', 'yu', 'yue', 'q', 'x']
...
def train_net(args):
    torch.manual_seed(7)
    np.random.seed(7)
    checkpoint = 'test_checkpoint_model.tar'
    start_epoch = 0
    best_loss = float('inf')
    ...
    if checkpoint is None:
        ...
        optimizer = TransformerOptimizer(
            torch.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=args.lr, betas=(0.9, 0.98),
                             eps=1e-09))
        ...
        pretrained_dict = {k: v for k, v in pretrained_dict.items() if
                           k in model_dict.keys() and v.size() == model_dict[
                               k].size()}  ###and ('encoder_v' not in k)}  ###
    ...
    for epoch in range(start_epoch, 1):
        l2r_wer_lrw, l2r_per_lrw, r2l_wer_lrw, r2l_per_lrw = valid_lrw(valid_loader=valid_loader_LRW, model=model,
                                                                       logger=logger)

        l2r_wer_lrw1000, l2r_per_lrw1000, r2l_wer_lrw1000, r2l_per_lrw1000 = valid_lrw1000(
            valid_loader=valid_loader_LRW1000, model=model, logger=logger)
    ...
    for data in tqdm(valid_loader):
        ...
        with torch.no_grad():
            ...
            for n in range(length):
                ...
                changdu = len(golds)
                preds = [total_phonemes[one] for one in predss_l2r[n].cpu().numpy()[:(changdu + 1)] if
                         one not in (sos_id, eos_id, -1)]
            ...
            for n in range(length_r2l):
                ...
                changdu = len(golds)
                preds = [total_phonemes[one] for one in predss_r2l[n].cpu().numpy()[:(changdu + 1)] if
                         one not in (sos_id, eos_id, -1)]
